...
class Jacobi_block():
    def __init__(self, num_node, load, mask, E, mu, resp, beta):
        ...
        self.rho = tf.stack([E[0],mu[0],E[1],mu[1]],0)
        self.E, self.mu = E, mu
        self.orig_E_1, self.orig_E_2 = tf.split(E, 2)
        self.orig_mu_1, self.orig_mu_2 = tf.split(mu, 2)
        self.E_1 = self.orig_E_1
        self.E_2 =  self.orig_E_2
        self.mu_1 = self.orig_mu_1
        self.mu_2 = self.orig_mu_2
    ...
    def get_optimizer(self):
        ...
        self.rho_grads = self.optimizer.compute_gradients(self.loss, var_list=[self.E, self.mu])
        self.rho_train_op = self.optimizer.apply_gradients(self.rho_grads)

        self.optimizer = tf.train.AdamOptimizer(rho_lr, beta1=0.5)
        self.E_grads = self.optimizer.compute_gradients(self.loss, var_list=self.E)
        self.E_train_op = self.optimizer.apply_gradients(self.E_grads)
        self.optimizer = tf.train.AdamOptimizer(rho_lr, beta1=0.5)
        self.mu_grads = self.optimizer.compute_gradients(self.loss, var_list=self.mu)
        self.mu_train_op = self.optimizer.apply_gradients(self.rho_grads)
...
if __name__ == "__main__":
    ...
    # [230 / 1e3, 0.36, 200 / 1e3, 0.25]
    E_pl = tf.Variable([0.23*1.1,0.2*1.1], tf.float32)
    mu_pl = tf.Variable([0.36*1.1,0.25*1.1], tf.float32)

    # build network
    beta = tf.constant(1.0, tf.float32)#controls the topology mass hyper-parameter
    jacobi = Jacobi_block(num_node, load_pl, mask_pl, E_pl, mu_pl, resp_pl, beta)
    ...
    for itr in tqdm(range(10001)):
        ...
        if flag:
            pass
            # if np.mean(np.abs(sess.run(jacobi.mask_grads, feed_dict_train))) < 1e-3:
            #     continue
            # sess.run(jacobi.mask_train_op, feed_dict_train)
        else:
            active_set = 0 # nothing is at boundary
            rho_val = sess.run([jacobi.orig_E_1, jacobi.orig_E_2, jacobi.orig_mu_1, jacobi.orig_mu_2])
            if rho_val[0]>0.5 or rho_val[0]<0 or rho_val[1]>0.5 or rho_val[1]<0:
                active_set = 1 # E at boundary
            elif rho_val[2]>0.5 or rho_val[2]<0 or rho_val[3]>0.5 or rho_val[3]<0:
                active_set = 2 # mu at boundary

            if active_set == 0:
                if np.mean(np.abs(sess.run([jacobi.rho_grads[0][0], jacobi.rho_grads[1][0]], feed_dict_train))) < 1e-3:
                    pass
                else:
                    sess.run(jacobi.rho_train_op, feed_dict_train)
            elif active_set == 1:
                jacobi.E_1 = tf.clip_by_value(jacobi.orig_E_1, 0, 1)
                jacobi.E_2 = tf.clip_by_value(jacobi.orig_E_2, 0, 1)
                if np.mean(np.abs(sess.run(jacobi.mu_grads[0], feed_dict_train))) < 1e-3:
                    pass
                else:
                    sess.run(jacobi.mu_train_op, feed_dict_train)
            elif active_set == 2:
                jacobi.mu_1 = tf.clip_by_value(jacobi.orig_mu_1, 0, 0.5)
                jacobi.mu_2 = tf.clip_by_value(jacobi.orig_mu_2, 0, 0.5)
                if np.mean(np.abs(sess.run(jacobi.E_grads[0], feed_dict_train))) < 1e-3:
                    pass
                else:
                    sess.run(jacobi.E_train_op, feed_dict_train)

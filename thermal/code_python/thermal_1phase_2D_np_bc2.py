...
def boundary_correct(x):
    ...
    x = np.pad(x[1:-1, 1:-1], ((1, 1), (1, 1)), "constant") # for boundary consideration
    return x
...
def main():
    ...
    u_hist = [np.zeros((num_node,num_node))] # because the solution is 66x68, 2 extra column is left for boundary condition
    loss_hist = []
    for i in range(3000):

...
%  for jj=1:size(a,2)
 ...
 [K,f] = conductivity_matrix(mesh, k,a);
%  Body_f{jj,1} = f;
...
mesh.node_inner_q = find(mesh.x(1,:) >= Element_size*(32-5) & mesh.x(1,:) <= Element_size*(32+5) & mesh.x(2,:)>=Element_size*(32-5) & mesh.x(2,:)<=Element_size*(32+5));
f(mesh.node_inner_q) = -1;
